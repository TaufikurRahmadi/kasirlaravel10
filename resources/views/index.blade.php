...
include 'koneksi.php';
$laki = mysqli_query($conn, "SELECT COUNT(jenis_kelamin) as laki_laki FROM data_mahasiswa where jenis_kelamin = 'laki-laki';");

$perempuan = mysqli_query($conn, "SELECT COUNT(jenis_kelamin) as perempuan FROM data_mahasiswa where jenis_kelamin = 'perempuan';");


// Fungsi untuk mendapatkan jumlah mahasiswa per jurusan
function getJumlahMahasiswaPerJurusan($conn)
{
	$query = "SELECT data_jurusan.nama_jurusan, COUNT(data_mahasiswa.id_mahasiswa) as jumlah_mahasiswa
              FROM data_mahasiswa
              RIGHT JOIN data_jurusan ON data_mahasiswa.id_jurusan = data_jurusan.id_jurusan
              GROUP BY data_jurusan.nama_jurusan";
	$result = mysqli_query($conn, $query);
	return $result;
}

// Fungsi untuk mendapatkan jumlah jurusan
function getJumlahJurusan($conn)
{
	$query = "SELECT COUNT(*) as jumlah_jurusan FROM data_jurusan";
	$result = mysqli_query($conn, $query);
	$row = mysqli_fetch_assoc($result);
	return $row['jumlah_jurusan'];
}

// Fungsi untuk mendapatkan jumlah mahasiswa berdasarkan jenis kelamin
function getJumlahJenisKelamin($conn)
{
	$query = "SELECT jenis_kelamin, COUNT(id_mahasiswa) as jumlah_mahasiswa
              FROM data_mahasiswa
...
{
	$query = "SELECT COUNT(*) as total_mahasiswa FROM data_mahasiswa";
	$result = mysqli_query($conn, $query);
	$row = mysqli_fetch_assoc($result);
...
$jumlah_mahasiswa_per_jurusan = getJumlahMahasiswaPerJurusan($conn);
$jumlah_jenis_kelamin = getJumlahJenisKelamin($conn);
								...
								<div class="card-body">
									<div class="media align-items-center">
										...
										<div class="media-body">
											<p class="fs-14 mb-2">Jumlah Jurusan
												<span class="title text-black font-w600">
													<?php echo getJumlahJurusan($conn); ?>
											</p></span>
										</div>
									</div>
								...
								<div class="card-body">
									<div class="media align-items-center">
										...
										<div class="media-body">
											...
											<span class="title text-black font-w600">
												L:
												<?php while ($row2 = $laki->fetch_assoc()) {
													echo $row2['laki_laki'] . "<br>";
												} ?>
												P:
												<?php while ($row3 = $perempuan->fetch_assoc()) {
													echo $row3['perempuan'] . "<br>";
												} ?>
										</div>
									</div>
									<div class="progress" style="height:5px;">
						...
						<div class="col-12">
							<div class="table-responsive">
								<table class="table table-bordered">
									...
									<tbody>
										<?php
										while ($row = mysqli_fetch_assoc($jumlah_mahasiswa_per_jurusan)) {
											echo "<tr>";
						...
						<div class="col-lg-12">
							<div class="card bg-gra"
								...
								<div class="card-body">
									<div class="row d-flex">
										<div class="col-md-8">
											<h4 class="text-bold text-weight-bolder"
												...
												<img src="https://unibamadura.ac.id/page/images/profil/1.png" width="28"
									...
									<div class="mt-4">
										<h5 class="mb-0" style="color: #008eff">
											<?php if (isset($_SESSION['id_operator'])): ?>
												<p class="mt-3">Selamat Datang,
													<?php echo $_SESSION['nama_operator']; ?>!
												</p>
											<?php endif; ?>
										</h5>
									</div>
